...
class SqlCompaction:
    ...
    def __process_table(self, catalog, database, table_name):
        # Check if GC handling is enabled in the config
        if self.config.gc_handling.enabled:
            # Check current GC status for the table
    ...
    def __check_gc_enabled(self, catalog, database, table_name):
        """Check if G.C. is enabled for the table
        
        Returns:
            bool or None: True if G.C. is enabled, False if G.C. is disabled, None if property not set
        """
        try:
            ...
            for row in result:
                if row.key.lower() in ("gc.enabled", "write.delete.parquet.compression-codec.enabled"):
    ...
    def __set_gc_enabled(self, catalog, database, table_name, enabled):
        """Set the G.C. enabled property for the table
        
        Args:
            catalog: Catalog name
            database: Database name
            table_name: Table name
            enabled: Boolean value to set the property to
        """
        try:
